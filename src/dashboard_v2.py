...
<html>
    <head>{%metas%}<title>Network Threat Monitor</title>{%favicon%}{%css%}</head>
    <body>
    <style>._dash-undo-redo{display:none !important;}</style>
    {%app_entry%}
    {%config%}
    {%scripts%}
    {%renderer%}
...
def update_dashboard(n):
    ...
    #-----donut chart-----
    if summary:
        donut_labels = [s["threat_level"] for s in summary]
        donut_values = [s["count"] for s in summary]
        donut_colors = [
            "#FF4444" if l == "Critical" else
            "#FFA500" if l == "Medium" else
            "#FFD700" for l in donut_labels
        ]
        donut = {
            "data": [{
                "type": "pie",
    ...
    return last_updated, total, critical, medium, low, fig, donut, table_data
